# KTAI — quy tắc làm việc trong repo này

Áp dụng cho mọi phiên KTAI làm việc trên cây KTAI (`~/KTAI`) và các project của chủ sở hữu.
...
- **Bảo toàn tính mạng của hệ thống.** `KTAI/core` là runtime đang chạy của KTAI: mọi thay đổi vào core phải đi qua `scripts/rebrand_core.py` hoặc patch có ghi lại, không sửa tay rồi quên.

## Cấu trúc cây KTAI
```
~/KTAI/
├── core/          # source runtime (fork từ Hermes core) — KHÔNG sửa tay tuỳ tiện
├── ktai/          # package CLI của KTAI (identity, agents, team, passthrough)
├── identity/      # lớp bản sắc: SOUL.md, AGENTS.md, agents/*.md, skins/, skills/
├── bin/           # launcher: ktai, ktai_entry.py
├── scripts/       # clone / rebrand / setup venv / seed home / verify — tái lập được
├── patches/       # diff + manifest của mọi sửa đổi trên core
├── venv/          # runtime riêng của KTAI (đã repoint khỏi ~/.hermes)
└── docs/          # ARCHITECTURE.md, KTAI-IDENTITY.md, REBRAND.md, AGENT-ROLES.md
```
Home dữ liệu: `~/.ktai/` (config.yaml, .env, skills, agents, memories, sessions, state.db, logs).
...
- Logic runtime mới → ưu tiên viết ở lớp `~/KTAI/ktai` (skill/CLI/plugin) thay vì sửa core; giữ core gần upstream để còn merge được fix.
