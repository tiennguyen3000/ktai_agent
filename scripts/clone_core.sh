#!/usr/bin/env bash
# KTAI — core acquisition step.
#
# The KTAI distribution is two things in one repo:
#   * the KTAI layer  (branch `main`)  — CLI, identity, scripts, docs
#   * the runtime core (branch `core`) — forked Hermes core with the KTAI identity layer
#
# This script materialises the runtime into <KTAI_ROOT>/core, from whichever source
# is available:
#
#   --mode local  rsync from the local Hermes checkout (fast, same-machine rebuilds)
#   --mode git    clone the `core` branch of the KTAI repo (fresh machine, no Hermes)
...
MODE="${MODE:-auto}"
REF="${REF:-core}"
REPO="${REPO:-https://github.com/tiennguyen3000/ktai_agent.git}"
...
usage: clone_core.sh [--mode auto|local|git] [--repo URL] [--ref BRANCH] [--src DIR] [--dst DIR]
  ...
  --mode git     clone the core branch from the KTAI repo ($REPO)
