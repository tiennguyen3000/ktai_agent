# KTAI — kiến trúc hệ thống (bản đồ thật, đo từ code)

Tài liệu này là Phase 0–1 của quá trình clone/rebuild: bản đồ kiến trúc của hệ thống
nguồn (Hermes Agent v0.21.1) **đo trực tiếp từ code**, kèm ghi chú KTAI giữ/đổi gì.

Bản gốc: `/Users/tiennk/.hermes/hermes-agent` (git install, upstream `a55c972e`,
local `ee35a462`, +65 carried commits). Bản KTAI: `/Users/tiennk/KTAI/core`.

## 1. Điểm vào & vòng lặp agent

```
~/.local/bin/ktai                      # launcher KTAI (bash) — set KTAI_HOME/HERMES_HOME
└── KTAI/bin/ktai_entry.py             # python entry (sys.path = core/ + KTAI/)
    └── ktai/cli.py:main()             # dispatch lệnh KTAI-native; còn lại passthrough
        └── os.execv(venv/bin/hermes)  # argv[0]="ktai" → core CLI, giữ nguyên fidelity
...
| Identity mặc định | `agent/prompt_builder.py::DEFAULT_AGENT_IDENTITY` ← **KTAI đã sửa** |
| Guidance tự-tri-thức | `HERMES_AGENT_HELP_GUIDANCE` (+ biến thể `_NO_SKILLS`) ← **KTAI đã sửa** |
| Persona người dùng | `SOUL.md` (home), seed bởi `hermes_cli/default_soul.py` ← **KTAI đã sửa** |
...
|---|---|
| Home profile-aware | `hermes_constants.py::get_hermes_home()` — override context → `KTAI_HOME` **(KTAI thêm)** → `HERMES_HOME` → default |
...
| Config | `~/.ktai/config.yaml` (settings) + `.env` (secrets) |
| Branding/theme | `hermes_cli/skin_engine.py` — skin YAML ở `<home>/skins/`, có `branding{}`, `banner_logo`, `tool_prefix` |
| Banner | `hermes_cli/banner.py` — logo + `format_banner_version_label()` ← **KTAI đã sửa** |
...
| `identity/*`, `ktai/*`, `bin/*`, `scripts/*`, `docs/*` | ❌ người viết | ✅ sửa trực tiếp |
