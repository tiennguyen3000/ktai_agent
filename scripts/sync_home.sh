#!/usr/bin/env bash
# ============================================================================
# KTAI home sync — one way, Hermes home -> KTAI home.
# ============================================================================
# KTAI's home is deliberately isolated from ~/.hermes (own sessions, memory, skills).
# Seeding copies the capability surface ONCE; after that the two homes drift apart.
# This script is the controlled way to pull later Hermes-side work into KTAI.
#
# What it syncs: skills/  memories/  plugins/   (same set the seeder inherits)
#
# Safety rules baked in:
#   * DRY RUN by default — nothing is written unless --apply is passed.
#   * Never overwrite by default: only files missing on the KTAI side are copied
#     (differences are still reported, so you can decide to re-run with --overwrite).
#   * skills/ktai is never touched: it is the pinned KTAI skill (ESSENTIAL_SKILLS).
#   * No deletions, ever. Files that exist only in KTAI stay (--show-extra lists them).
#
# Usage:
...
#   bash scripts/sync_home.sh --show-extra         # also list KTAI-only files
#
# Env overrides: SRC_HOME (default ~/.hermes), DST_HOME (default $KTAI_HOME or ~/.ktai)
#
...
DST_HOME="${DST_HOME:-${KTAI_HOME:-$HOME/.ktai}}"
...
[[ -d "$DST_HOME" ]] || die "KTAI home not found: $DST_HOME — install/seed first (scripts/install.sh)"
...
echo "KTAI home sync  ($([[ $APPLY -eq 1 ]] && echo APPLY || echo DRY RUN))"
echo "  from: $SRC_HOME"
echo "  to:   $DST_HOME"
...
for area in "${AREA_LIST[@]}"; do
  ...
  area_flags=()
  # The pinned KTAI skill must never be replaced by a Hermes-side copy. Patterns are
  # relative to the transfer root, so for the skills area that is top-level `ktai/`.
  [[ "$area" == "skills" ]] && area_flags+=(--exclude 'ktai/')
...
echo
echo "Tổng: $total_new file mới, $total_diff file khác nội dung$([[ $SHOW_EXTRA -eq 1 ]] && echo ", $total_extra file chỉ có bên KTAI")"
if [[ $APPLY -eq 0 ]]; then
  echo "DRY RUN — chưa ghi gì. Thêm --apply để copy ($([[ $OVERWRITE -eq 1 ]] && echo 'chế độ overwrite' || echo 'chỉ file mới'))."
else
  echo "Đã copy ($([[ $OVERWRITE -eq 1 ]] && echo 'overwrite' || echo 'chỉ file mới')). Không xoá gì, không đụng skills/ktai."
  echo "Kiểm tra lại: ktai selfcheck   (skill mới nhận ở phiên kế tiếp)"
fi
