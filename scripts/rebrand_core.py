#!/usr/bin/env python3
"""KTAI rebrand patcher — surgical, idempotent, manifest-driven.

Applies the KTAI identity layer to the forked core at ~/KTAI/core.
...
Design rules (why this file exists instead of a global sed):
  * Only IDENTITY / USER-FACING surfaces are rewritten. Internal module names
    ...
    upstream fixes. See KTAI/docs/REBRAND.md for the full rationale.
...
# KTAI inherits Hermes' reply-sizing / no-filler contract, only the identity changes.
_ORIG_ID = (
    '    "You are Hermes Agent, built by Nous Research. Be direct: match the length of your reply to the weight of the ask "'
)
_NEW_ID = (
    '    "You are KTAI (Khánh Tiển AI) — the personal AI engineering agent of Nguyễn Khánh Tiển, running on the '
...
_NEW_GUIDANCE = (
    '    "You run on KTAI (Khánh Tiển AI), a personal AI engineering agent built on the Hermes core by Nous "\n'
    '    "Research. When the user needs help with KTAI itself — configuring, setting up, using, extending, or "\n'
    '    "troubleshooting it — or when you need to understand your own features, tools, or capabilities, the `ktai` "\n'
    '    "skill holds the KTAI-specific commands and conventions: load it with skill_view(name=\'ktai\') first. For "\n'
...
_SOUL_NEW_COMMENT = (
    "# KTAI: kept identical to agent/prompt_builder.py's DEFAULT_AGENT_IDENTITY (KTAI variant): _ensure_default_soul_md()"
)
...
_SCAFFOLD_NEW = (
    '    "# KTAI Persona\\n\\n<!--\\nThis file defines the agent\'s personality and tone.\\n"\n'
    '    "The agent will embody whatever you write here.\\nEdit this to customize how KTAI communicates with you.\\n\\n"'
)
...
REPLACEMENTS = [
    ...
    # 3. Let KTAI's own skill drive the rich guidance variant.
    {
        ...
        "new": '    if "skill_view" in (agent.valid_tool_names or set()) and (\n'
               '            "- ktai:" in skills_prompt or "- hermes-agent:" in skills_prompt\n'
               '    ):',
        "why": "KTAI skill drives guidance variant",
    },
    ...
    # 5. Banner: KTAI wordmark + hero art + version label.
    {
        ...
        "new": ("from hermes_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE\n"
                "from ktai_branding import KTAI_HERO, KTAI_LOGO, version_label as _ktai_version_label"),
        "why": "import KTAI branding",
    },
    {
        "file": "hermes_cli/banner.py",
        "old": '    left_lines = ["", getattr(_bskin, "banner_hero", None) or HERMES_CADUCEUS, ""]',
        "new": '    left_lines = ["", getattr(_bskin, "banner_hero", None) or KTAI_HERO, ""]',
        "why": "banner hero art",
    },
    {
        "file": "hermes_cli/banner.py",
        "old": '        console.print(getattr(_bskin, "banner_logo", None) or HERMES_AGENT_LOGO)',
        "new": '        console.print(getattr(_bskin, "banner_logo", None) or KTAI_LOGO)',
        "why": "banner wordmark logo",
    },
    {"file": "hermes_cli/banner.py", "old": '    base = f"Hermes Agent v{VERSION} ({RELEASE_DATE})"',
     "new": '    base = _ktai_version_label(VERSION, RELEASE_DATE)', "why": "banner version label"},
    # 6. Fallback branding for the built-in default skin.
    {
        ...
        "new": ("from hermes_constants import get_hermes_home\nfrom ktai_branding import KTAI_BRANDING, KTAI_SPINNER"),
        "why": "import KTAI branding",
    },
    {
        "file": "hermes_cli/skin_engine.py",
        "old": '_HERMES_BRANDING: Dict[str, str] = _branding(\n    "Hermes", "⚕", "Goodbye! ⚕", prompt="❯", help_header="(^_^)? Available Commands")',
        "new": '_HERMES_BRANDING: Dict[str, str] = dict(KTAI_BRANDING)  # KTAI identity (see ktai_branding)',
        "why": "default-skin branding",
    },
    # 7. KTAI_HOME as a first-class home override (identity at the runtime level).
    {
        ...
        "new": ('    override = get_hermes_home_override()\n'
                '    if override:\n'
                '        return Path(override)\n'
                '    # KTAI layer: KTAI_HOME is the canonical home for the KTAI distribution and wins\n'
                '    # over HERMES_HOME (the core mechanism it is implemented on top of).\n'
                '    _ktai_home = os.environ.get("KTAI_HOME", "").strip()\n'
                '    if _ktai_home:\n'
                '        return Path(_ktai_home)\n'
                '    if not os.environ.get("HERMES_HOME", "").strip():'),
        "why": "KTAI_HOME override",
    },
    # 8. CLI user-facing strings.
    {"file": "cli.py", "old": '        tiny_line = "⚕ NOUS HERMES"', "new": '        tiny_line = "⚡ KTAI"', "why": "banner tiny line"},
    {
        "file": "cli.py",
        "old": ('        tiny_line = _skin.get_branding("agent_name", "Hermes Agent") if _skin else "Hermes Agent"'),
        "new": '        tiny_line = _skin.get_branding("agent_name", "KTAI") if _skin else "KTAI"',
        "why": "banner tiny line (fallback)",
    },
    {"file": "cli.py", "old": '        version_line = f"Hermes Agent v{_version} ({_release_date})"',
     "new": '        version_line = _ktai_version_label(_version, _release_date)',
    ...
    {"file": "cli.py", "old": '        _welcome_text = "Welcome to Hermes Agent! Type your message or /help for commands."',
     "new": '        _welcome_text = "KTAI ready — Personal AI Engineering Agent. Type your message or /help for commands."',
     "why": "welcome fallback"},
    {"file": "cli.py", "old": '"""Hermes Agent CLI — interactive terminal interface (``python cli.py --help`` for usage)."""',
     "new": '"""KTAI CLI — interactive terminal interface for the KTAI personal AI engineering agent."""',
     "why": "module docstring"},
    {"file": "cli.py", "old": '    """Interactive REPL for the Hermes Agent."""',
     "new": '    """Interactive REPL for KTAI."""', "why": "class docstring"},
    {"file": "cli.py", "old": "    Hermes Agent CLI - Interactive AI Assistant",
     "new": "    KTAI — Personal AI Engineering Agent (CLI)", "why": "help text"},
    # 9. Pin KTAI's own skill as essential (like hermes-agent): the system prompt's
    #    guidance slot depends on it being installed, so it must not be disable-able.
    {
        ...
        "new": 'ESSENTIAL_SKILLS: frozenset = frozenset({"hermes-agent", "ktai"})  # "ktai": pinned by the KTAI layer',
        "why": "KTAI skill is essential",
    },
]
...
TEST_ALIGNMENT = [
    {"file": "tests/hermes_cli/test_banner.py",
     ...
     "new": '    assert "KTAI v" in raw, "Version label missing from title"',
    ...
    {"file": "tests/hermes_cli/test_skin_engine.py",
     ...
     "new": '        assert skin.get_branding("agent_name") == "KTAI"',
    ...
    {"file": "tests/hermes_cli/test_startup_fast_guards.py",
     ...
     "new": '    for field in ("KTAI v", "Install directory:", "Python:", "OpenAI SDK:"):',
     "why": "test alignment: fast --version identity"},
    {"file": "tests/hermes_cli/test_startup_fast_guards.py",
     "old": '    assert "Hermes Agent v" in result.stdout',
     "new": '    assert "KTAI v" in result.stdout',
    ...
    {"file": "tests/agent/test_phantom_tool_references.py",
     ...
     "new": '        assert "skill_view(name=\'ktai\')" in HERMES_AGENT_HELP_GUIDANCE',
     "why": "test alignment: guidance points at the KTAI skill"},
]
...
CLI_IMPORT_NEW = ("from hermes_cli.banner import format_banner_version_label\n"
                  "from ktai_branding import version_label as _ktai_version_label")
...
def write_audit(applied: list[dict], touched: dict) -> None:
    ...
    for rel in files:
        ...
        else:  # new file added by the KTAI layer
            base = []
    ...
    header = (
        "# KTAI rebrand patch — the complete KTAI-vs-upstream diff for the core.\n"
...
def main() -> int:
    ...
    if CLI_IMPORT_NEW not in cli_text:
        if CLI_IMPORT_OLD not in cli_text:
            ...
            applied.append({"file": "cli.py", "why": "import KTAI version label"})
    ...
    print(f"KTAI rebrand applied: {len(applied)} edits across {len(touched)} files")
    for a in applied:
    ...
    chk = subprocess.run(
        [sys.executable, "-c", "import ktai_branding as b; print(b.KTAI_NAME, b.version_label('0.21.1'))"],
