# KTAI Engineering Agents — 6 vai chuyên trách

Charter nằm ở `~/.ktai/agents/<role>.md` (nguồn: `identity/agents/`). Mỗi charter gồm
...
```bash
ktai agents                       # liệt kê cả 6
ktai agents debug                 # in charter đầy đủ
ktai team debug "lỗi X khi chạy Y"     # giao việc → agent chạy process riêng
ktai team review --show                # xem prompt sẽ gửi (không chạy)
KTAI_TEAM_DRY_RUN=1 ktai team test "…"  # in prompt rồi thoát
```

Trong phiên KTAI, có thể giao việc qua `delegate_task` và dán charter tương ứng làm context.
`ktai team` chạy `venv/bin/hermes chat -q` trong process riêng: session riêng, đầy đủ tool,
...
`python3 scripts/seed_home.py` để cài vào `~/.ktai/agents/`, rồi `ktai agents` để kiểm.
