"""KTAI CLI — KTAI-native subcommands + faithful passthrough to the core CLI.

  ktai                     interactive session (core CLI)
  ktai <anything else>     passed through to the core, argv[0] = "ktai"
  ktai version             KTAI + core version
  ktai identity            identity card, home, team roster
  ktai agents [role]       list the engineering agents / show one charter
  ktai team <role> <task>  delegate a task to one engineering agent
  ktai selfcheck           verify the KTAI installation end to end
  ktai home                print the KTAI data home
"""
...
from . import CORE, IDENTITY, KTAI_NAME, KTAI_VERSION, ROOT, core_entry, ktai_home
from . import brain


def _print_version() -> int:
    from ktai_branding import version_label

    print(f"{KTAI_NAME} v{KTAI_VERSION} — Personal AI Engineering Agent")
    print(f"core: {version_label(brain._core_version(), brain._core_release())}")
    print(f"home: {ktai_home()}")
...
def _team(argv: list[str]) -> int:
    if not argv or argv[0] in ("-h", "--help"):
        print('usage: ktai team <role> "<task>"   |   ktai team <role> --show')
    ...
    if os.environ.get("KTAI_TEAM_DRY_RUN") == "1":
        print(prompt)
        return 0

    print(f"── {KTAI_NAME} · {role['title']} ─────────────────────────────")
...
def _selfcheck() -> int:
    ...
    home = ktai_home()
    checks.append(("KTAI home exists", home.is_dir(), str(home)))
    ...
    def _skin_ok() -> tuple[bool, str]:
        ...
        try:
            ...
            ok = configured == "ktai" and skin.name == "ktai" and skin.get_branding("agent_name", "") == KTAI_NAME
    ...
    def _identity_ok() -> tuple[bool, str]:
        try:
            ...
            return "You are KTAI" in DEFAULT_AGENT_IDENTITY, "system-prompt identity"
        except Exception as exc:  # noqa: BLE001
            return False, f"identity import failed: {exc}"

    def _isolation_ok() -> tuple[bool, str]:
        # A KTAI run must never resolve its home to ~/.hermes.
...
# ── KTAI-native dispatch (everything else goes to the core) ───────────────────
_TABLE = {
    ...
    "home": lambda argv: (print(ktai_home()), 0)[1],
}


def passthrough(argv: list[str]) -> int:
    """Exec the core CLI with argv[0]='ktai' — full fidelity, same process image."""
    entry = core_entry()
    if entry.is_file():
        os.execv(str(entry), ["ktai", *argv])  # replaces this process
